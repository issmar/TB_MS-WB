...
def exportar_respuestas_wa():
    GENERADOS_DIR = "./Generados"
    DESTINO_DIR = "./RespuestasWA"
    DESTINO_FILE = os.path.join(DESTINO_DIR, "RespuestasWA.xlsx")
    ...
    col_estado = "Estado Respuesta WA"
    ...
    if df_respondidos.empty:
        print("ℹ️ No hay registros con respuesta WA.")
        return
    ...
    if os.path.exists(DESTINO_FILE):
        ...
        if all(col in df_hist.columns for col in claves) and all(col in df_respondidos.columns for col in claves):
